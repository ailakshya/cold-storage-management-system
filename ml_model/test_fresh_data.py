"""
Fresh Database Data Workflow Test
=================================
Tests the recommendation system on FRESH data exported from production database.
"""
...
def run_fresh_data_test():
    """Test on fresh data from production database"""
    print("=" * 70)
    print("FRESH DATABASE DATA WORKFLOW TEST")
    print("=" * 70)

    # Load FRESH data from production database export
    print("\nLoading fresh data from production database export...")
    ...
    print(f"Loaded {len(inventory)} inventory records (fresh from DB)")
    print(f"Loaded {len(customer_data)} customer pattern records (fresh from DB)")
    ...
    print("\n" + "─" * 70)
    print("FRESH INVENTORY SUMMARY")
    print("─" * 70)

    room_stats = defaultdict(lambda: {'gatars': 0, 'items': 0})
    total_items = 0
    for key, data in model.inventory_data.items():
        room = data['room_no']
        room_stats[room]['gatars'] += 1
        room_stats[room]['items'] += data['items']
        total_items += data['items']

    for room in sorted(room_stats.keys()):
        print(f"  Room {room}: {room_stats[room]['gatars']} gatars, {room_stats[room]['items']:,} items")
    print(f"  TOTAL: {total_items:,} items across {len(model.inventory_data)} gatars")
    ...
    config_metrics = defaultdict(lambda: {
        'total': 0, 'passed': 0, 'failed': 0, 'times': [], 'errors': defaultdict(int)
    })
    ...
    print(f"\n  Data Source:     FRESH from Production Database")
    ...
    print(f"\n" + "=" * 70)
    if overall_pct == 100:
        print("  *** ALL TESTS PASSED ON FRESH DB DATA ***")
        print("  *** SYSTEM VERIFIED WITH PRODUCTION DATA ***")
    elif overall_pct >= 99:
        print("  *** TESTS MOSTLY PASSED - MINOR ISSUES ***")
    elif overall_pct >= 95:
